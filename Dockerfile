# Builds the HN Android app (debug + release APK, unit tests) with
# podman/docker.
#
#   podman compose build
#   podman compose run --rm build
#
# The APKs and the unit test results are copied into ./artifacts by the
# compose service. The release build is only signed when
# release.properties (+ the keystore it points at) exist, see
# app/build.gradle.
#
# A standalone builder image (SDK + JDK + emulator) for ad-hoc builds:
#   podman build --target builder -t hn-android-builder .
...
RUN ./gradlew --no-daemon :app:assembleDebug :app:assembleRelease :app:testDebugUnitTest
...
COPY --from=builder /work/app/build/outputs/apk/release/app-release-*.apk /artifacts/
